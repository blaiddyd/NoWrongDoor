...
$('#search').click(() => {
  ...
  const catInput = document.querySelector('#cat').value;
  const subInput = document.querySelector('#sub').value;
  if (catInput === null || catInput === '', subInput === null || subInput === '', ageInput === null || ageInput === '') {
    alert('Please Fill All Required Fields');
  }

  const results = orgsArray.filter(org => org.OrgSuburb === subInput
  ...
  const $finalTable = $('#filtered').append($tablebody);
...
$(document).ready(() => {
  ...
  const $tablebody = $('<tbody></tbody>');
  for (let i = 0; i < orgsArray.length; i += 1) {
    const oneOrg = orgsArray[i];
    const $row = $('<tr></tr>');
    $row.append($('<td></td>').text(oneOrg.OrgName));
    $row.append($('<td></td>').text(oneOrg.OrgField));
    $row.append($('<td></td>').text(oneOrg.OrgSuburb));
